...
import { INITIAL_CATEGORIES, INITIAL_BRANDS, INITIAL_USERS, INITIAL_SETTINGS } from '../constants';
import { loadLocalBranches, saveLocalBranches, upsertLocalBranch } from '../services/localBranches';
...
export const StoreProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  const [branches, setBranches] = useState<Branch[]>(() => loadLocalBranches());
  const [currentBranch, setCurrentBranch] = useState<Branch>(() => loadLocalBranches()[0]);
  ...
  const addBranch = (branchInput: Omit<Branch, 'id'> & { id?: string }) => {
    ...
    const updated = upsertLocalBranch(localBranch);
    setBranches(updated);
